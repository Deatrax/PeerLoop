...
for(const [account,heading] of [['Nasrin','Only what needs your call.'],['Admin','']] as const){
  test(`${account} role renders its operational screens`,async({page})=>{
    ...
    else {await expect(page.getByRole('button',{name:'Admin',exact:true})).toBeVisible();await page.getByRole('button',{name:'Analytics',exact:true}).click();}
