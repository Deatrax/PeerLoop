...
test('staff digests are role-scoped, include unresolved work and are idempotent',()=>{
  ...
  for(const n of state.notifications.filter(n=>n.event_type.startsWith('digest.'))){assert.ok(state.space_memberships.some(m=>m.user_id===n.user_id&&m.space_id===n.space_id&&m.role!=='STUDENT'));assert.match(n.payload.text,/Unresolved this week/);}
});
test('delivery uses real email for staff digests and honors opt-out',async()=>{
  const state=seedDatabase(now);buildStaffDigests(state,now);const n=state.notifications.find(n=>n.event_type.startsWith('digest.'))!;const user=state.users.find(u=>u.id===n.user_id)!;
